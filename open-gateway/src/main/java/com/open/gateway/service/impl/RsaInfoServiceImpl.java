...
@Service
public class RsaInfoServiceImpl {

    private static final Logger logger = LoggerFactory.getLogger(RsaInfoServiceImpl.class);
    ...
    public GatewayRsa selectGatewayRsa(String organizationId) {
        String cacheKey = String.format("gateway:rsa:security:%s", organizationId);
        GatewayRsa gatewayRsa = this.redisCacheDaoService.read(cacheKey, GatewayRsa.class);
        if (gatewayRsa == null) {
            gatewayRsa = this.gatewayRsaMapper.selectByPrimaryKey(organizationId);
            if (gatewayRsa != null) {
